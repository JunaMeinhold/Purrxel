namespace Purrxel.Graphics.Graph
{
    public class ResourceDescriptor<T> : IResourceDescriptor<T>, IResourceDescriptor where T : struct
    {
        ...
        private Func<T, IDisposable> constructor;
        ...
        public ResourceDescriptor(T desc, ResourceRef @ref, GraphResourceContainer? container, Func<T, IDisposable> constructor, ResourceCreationFlags flags)
        {
            this.desc = desc;
        ...
        public Func<T, IDisposable> Constructor { get => constructor; set => constructor = value; }

        public TType Construct<TType>(List<TType> group) where TType : class, IDisposable
        {
            var resource = (TType)Constructor(Desc);
            Ref.Value = resource;
            lock (group)
            {
                group.Add(resource);
