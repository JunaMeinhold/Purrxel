namespace Purrxel.Engine.Graphics.D3D11
{
    ...
    public struct BlendDescription : IEquatable<BlendDescription>
    {
        ...
        public BlendDescription(Blend sourceBlend, Blend destinationBlend, Blend srcBlendAlpha, Blend destBlendAlpha, BlendOp blendOperation, BlendOp blendOperationAlpha)
            : this()
        {
            AlphaToCoverageEnable = false;
            IndependentBlendEnable = false;
        ...
        public BlendDescription(Blend sourceBlend, Blend destinationBlend, Blend srcBlendAlpha, Blend destBlendAlpha, BlendOp blendOperation, BlendOp blendOperationAlpha, LogicOp logicOperation)
            : this()
        {
            AlphaToCoverageEnable = false;
            IndependentBlendEnable = false;
        ...
        public static implicit operator BlendDesc1(BlendDescription desc)
        {
            ...
            for (int i = 0; i < SimultaneousRenderTargetCount; i++)
            {
                result.RenderTarget[i] = desc.RenderTarget[i];
            }
        ...
        {
            return renderTarget.BlendOperationAlpha != BlendOp.Add
                    || renderTarget.SourceBlendAlpha != Blend.One
                    || renderTarget.DestinationBlendAlpha != Blend.Zero
                    || renderTarget.BlendOperation != BlendOp.Add
