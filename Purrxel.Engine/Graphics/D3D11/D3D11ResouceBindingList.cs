namespace Purrxel.Engine.Graphics.D3D11
{
    ...
    public unsafe class D3D11ResourceBindingList : DisposableBase
    {
        ...
        public D3D11ResourceBindingList(GraphicsPipeline pipeline)
        {
            pipeline.AddRef();
        ...
        public D3D11ResourceBindingList(ComputePipeline pipeline)
        {
            pipeline.AddRef();
            pipeline.OnCompile += OnPipelineCompile;
            this.pipeline = pipeline;
        ...
        {
            Clear();
            if (pipeline is GraphicsPipeline graphicsPipeline)
            {
            ...
            if (pipeline is ComputePipeline computePipeline)
            {
        ...
        public void SetCBV(string name, void* cbv)
        {
            for (int i = 0; i < rangesCBVs.Count; i++)
            {
                rangesCBVs.GetPointer(i)->TrySetByName(name, cbv);
            }
        }
        ...
        public void SetSampler(string name, void* sampler)
        {
            for (int i = 0; i < rangesSamplers.Count; i++)
            {
                rangesSamplers.GetPointer(i)->TrySetByName(name, sampler);
            }
        }
