namespace Purrxel.Engine.Mathematics.Noise
{
    ...
    138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180
        ...
        public float OctavePerlin1D(float x, int octaves, float persistence, float amplitude)
        {
            float total = 0;
            float frequency = 1;
            ...
            {
                total += SaturateClamp01(Perlin1D(x * frequency)) * amplitude;
        ...
        public float OctavePerlin2D(float x, float y, int octaves, float persistence, float amplitude)
        {
            float total = 0;
            float frequency = 1;
            for (int i = 0; i < octaves; i++)
        ...
        public float OctavePerlin3D(float x, float y, float z, int octaves, float persistence, float amplitude)
        {
            ...
            for (int i = 0; i < octaves; i++)
            {
                total += SaturateClamp01(Perlin3D(x * frequency, y * frequency, z * frequency)) * amplitude;
